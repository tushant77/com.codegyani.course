package com.codegyani.course.filter;
...
import com.codegyani.course.security.JwtTokenUtil;
...
//		if(!(excludedUrls.stream().com.codegyani.course.filter(url-> request.getRequestURL().toString().toLowerCase().contains(url.toLowerCase())).count() > 0) && !request.getMethod().equals("OPTIONS")) {
